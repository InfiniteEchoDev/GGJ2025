...
for (var i = 0; i < width -1; i++)		// Stop before the end because you can't reference points of
{
	for (var j = 0; j < height -1; j++;)
	{
		...
		var f2 = ds_grid_get(grid, i+1, j+1);	// Btm right
		var f3 = ds_grid_get(grid,i, j+1);		// Btm left
		
		function l(a,b)		// function for linear interpolations
		{
			return a/(a-b);		// return 0.5 to disable linear interpolation	
								// would assume lines connect at mid point
		}
		...
		state = getState(f0,f1,f2,f3);		// state variable to store the base 10 representation of ...
		
		var w = 4;			// Line width
		var m = 2;			// circle radius ( multiplier of line width, 1 for no visibility
